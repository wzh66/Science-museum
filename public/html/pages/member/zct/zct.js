...
app.config(['$routeProvider', '$locationProvider', function ($routeProvider, $locationProvider) {
    ...
    // 设定路由
    $routeProvider
        .when('/member/zct', { //app首页
            templateUrl: 'pages/member/zct/zct.html',
            controller: "memberZctController"
        });
}]).controller('memberZctController', ['$scope', 'authSvc', 'subjectSvc', 'accountSvc', function ($scope, authSvc, subjectSvc, accountSvc) {
    ...
    $scope.getList = function () {
        accountSvc.list($scope.key, $scope.selected.value, 'zct', $scope.tradeType, $scope.page.page).then(function success(res) {
            if (res.code === '0000') {
                console.log(res);
    ...
    $scope.getData = function () {
        accountSvc.get($scope.key, $scope.selected.value, 'zct').then(function success(res) {
            console.log(res);
            if (res.code === '0000') {
                $scope.noExist = false;
